...
namespace WTP.DAL.Repositories.ConcreteRepositories
{
    internal class HistoryRepository : RepositoryBase<History>, IRepository<History>
    {
        ...
        {
            return await _context.Histories.Include(t => t.Admin)
            .Include(p => p.AppUser).Include(p => p.Operation).FirstOrDefaultAsync(_ => _.Id == id);
        }

        public override async Task<IEnumerable<History>> GetAllAsync()
        {
            return await _context.Histories.Include(t => t.Admin)
            .Include(p => p.AppUser).Include(p => p.Operation).ToListAsync();
                //_context.Histories.ToListAsync();
        }
    }
}
