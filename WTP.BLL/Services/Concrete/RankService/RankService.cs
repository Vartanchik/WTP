    ...
    public class RankService : IRankService
    {
        ...
        public async Task<IList<RankDto>> GetRanksListAsync()
        {
            var listOfranks = from rank in await _uow.Ranks.AsQueryable().ToListAsync()
                                join game in await _uow.Games.AsQueryable().ToListAsync() on rank.GameId equals game.Id
        ...
        public async Task<RankDto> FindAsync(int rankId)
        {
            return _mapper.Map<RankDto>(await _uow.Ranks.GetAsync(rankId));
        }
    }
}
